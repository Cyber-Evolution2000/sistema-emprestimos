...
class AdminSystemFirebase {
    ...
    async carregarDados() {
        try {
            ...
            // Carregar dados de exemplo se Firebase falhar
    ...
    async salvarCliente() {
        ...
        try {
            ...
            if (clienteExistente) {
                ...
                await dbService.atualizarCliente(cliente.cpf, cliente);
            } else {
    ...
    async excluirCliente(cpf) {
        if (confirm('Tem certeza que deseja excluir este cliente?')) {
            try {
                await dbService.excluirCliente(cpf);
                await this.carregarDados();
    ...
    atualizarUsuarioLogado() {
        ...
        if (usuario) {
            // Adiciona informação do usuário no sidebar
            const userInfo = document.createElement('div');
            userInfo.className = 'px-3 py-2 text-white small border-top border-white mt-3';
            ...
            if (sidebar) {
                ...
                const oldInfo = document.getElementById('user-info');
